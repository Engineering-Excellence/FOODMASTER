...
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/balance.css">
<script src="${pageContext.request.contextPath}/js/balance.js" type="text/javascript" defer></script>
...
<c:if test="${(page <= 0 || Math.ceil(balanceList.size() / 10) < page) && balanceList.size() > 0}">
    <%-- 이때, keyword 값이 null이 아니면, 검색을 통한 데이터이므로, 검색 현황을 유지시킨 상태에서 1페이지로 리다이렉트 --%>
    <c:if test="${keyword != null}">
        <c:redirect url="/balance/list?searchOption=${searchOption}&keyword=${keyword}&page=1"/>
    </c:if>
    <c:if test="${keyword == null}">
        ...
        <div class="search-input-container">
            ...
            <div class="search-select-type-wrapper btn-group btn-group-toggle" data-toggle="buttons">
                <label class="btn btn-outline-secondary active">
                    <input type="radio" name="select-type" id="edit" value="1" checked> 매출수정
                </label>
                <label class="btn btn-outline-secondary">
                    <input type="radio" name="select-type" id="delete" value="2"> 매출삭제
                </label>
            </div>
                    ...
                    <c:if test="${balanceList.size() > 0}">
                        ...
                        <c:forEach var="i" begin="${Math.floor((page - 1) / 10) * 10}"
                            ...
                            <c:if test="${page != i + 1}">
                                ...
                                <c:if test="${keyword == null}">
                                    <li class="page-item"><a class="page-link"
                                                             href="/balance/list?page=${i + 1}">${i + 1}</a>
                                    </li>
                                </c:if>
                            </c:if>
                        ...
                        <c:if test="${Math.floor((page - 1) / 10) < Math.floor(balanceList.size() / 100)}">
                            <li class="page-item">
                                    <%-- 페이지가 1, 11, 21, 31 등에서부터 시작할 수 있게 조절하는 부분 --%>
                                <c:if test="${keyword != null}">
                                    <a class="page-link"
